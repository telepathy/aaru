...
function crStep3() {
  return {
    content: `
      ...
      <div class="cr-section"><div class="cr-section-title">额外信息（选填，最多1024字符）</div>
        <textarea id="cr-extra-info" maxlength="1024" rows="3" style="width:100%;padding:8px;border:1px solid var(--border);border-radius:4px;font-size:13px;resize:vertical" placeholder="可填写发布相关的补充说明..." oninput="crSetExtraInfo(this.value)">${escapeHtml(crExtraInfo)}</textarea>
        <div style="font-size:11px;color:var(--text-muted);margin-top:4px;text-align:right"><span id="cr-extra-info-count">${crExtraInfo.length}</span>/1024</div>
      </div>
...
window.crSetExtraInfo = function(v) {
  crExtraInfo = v;
  const countEl = document.getElementById('cr-extra-info-count');
  ...
  return {
    content: `
      <div class="cr-section"><div class="cr-section-title">变更内容</div>
        ...
        ${crExtraInfo?`<div style="margin-bottom:8px"><strong>额外信息:</strong> <span style="white-space:pre-wrap">${escapeHtml(crExtraInfo)}</span></div>`:''}
...
window.crSubmitRelease = async function() {
  ...
  try {
    await api('/releases', { method:'POST', body:JSON.stringify({ title:crTitle, deploy_unit_code:crSelectedDU.code, blueprint_id:crSelectedBP.id, changes, extra_info:crExtraInfo }) });
    toast('发布单创建成功','success');
    crStep=1; crChanges={}; crExtraFields=[]; crPerEnvMode=new Set(); crPerEnvVals={}; crExtraInfo = '';
    loadPage('releases');
